...
namespace SQL
{
	...
	{
		const int ret = sqlite3_open_v2(apFilename, &m_pSQLite, aFlags, apVfs);
		if(SQLITE_OK != ret)
		{
			const SQL::Exception exception(m_pSQLite, ret); // must create before closing
			sqlite3_close(m_pSQLite); // close is required even in case of error on opening
			throw exception;
		}
		if(aBusyTimeoutMs > 0)
		{
	...
	Database::Database(const std::string& aFilename,
		...
		mFilename(aFilename)
	{
		const int ret = sqlite3_open_v2(aFilename.c_str(), &m_pSQLite, aFlags, aVfs.empty() ? NULL : aVfs.c_str());
		if(SQLITE_OK != ret)
		{
			const SQL::Exception exception(m_pSQLite, ret); // must create before closing
			sqlite3_close(m_pSQLite); // close is required even in case of error on opening
	...
	{
		const int ret = sqlite3_close(m_pSQLite);
	...
	{
		const int ret = sqlite3_busy_timeout(m_pSQLite, aBusyTimeoutMs);
		check(ret);
	}
	...
	{
		const int ret = sqlite3_exec(m_pSQLite, apQueries, NULL, NULL, NULL);
		check(ret);
	...
	{
		return sqlite3_total_changes(m_pSQLite);
	}
	...
	{
		return sqlite3_extended_errcode(m_pSQLite);
	}

	// Return UTF-8 encoded English language explanation of the most recent failed API call (if any).
	const char* Database::getErrorMsg() const noexcept // nothrow
	{
		return sqlite3_errmsg(m_pSQLite);
	}
		...
		void(*apDestroy)(void *))
	{
		...
		const int ret = sqlite3_create_function_v2(m_pSQLite, apFuncName, aNbArg, TextRep,
	...
	void Database::loadExtension(const char* apExtensionName, const char *apEntryPointName)
	{
		...
		int ret = sqlite3_db_config(m_pSQLite, SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION, 1, NULL);
#else
		int ret = sqlite3_enable_load_extension(m_pSQLite, 1);
#endif
		check(ret);

		ret = sqlite3_load_extension(m_pSQLite, apExtensionName, apEntryPointName, 0);
		check(ret);
#endif
	}
	...
	void Database::key(const std::string& aKey) const
	{
		...
		if(pass_len > 0)
		{
			const int ret = sqlite3_key(m_pSQLite, aKey.c_str(), pass_len);
			check(ret);
		}
#else // SQLITE_HAS_CODEC
		if(pass_len > 0)
		{
			const SQL::Exception exception("No encryption support, recompile with SQLITE_HAS_CODEC to enable.");
			throw exception;
		}
	...
	void Database::rekey(const std::string& aNewKey) const
	{
		...
		if(pass_len > 0)
		{
			const int ret = sqlite3_rekey(m_pSQLite, aNewKey.c_str(), pass_len);
			check(ret);
		}
		else
		{
			const int ret = sqlite3_rekey(m_pSQLite, nullptr, 0);
			check(ret);
		}
#else // SQLITE_HAS_CODEC
		const SQL::Exception exception("No encryption support, recompile with SQLITE_HAS_CODEC to enable.");
	...
	const bool Database::isUnencrypted(const std::string& aFilename)
	{
		if(aFilename.length() > 0)
		{
			...
			{
				const SQL::Exception exception("Error opening file: " + aFilename);
				throw exception;
			}
			return strncmp(header, "SQLite format 3\000", 16) == 0;
		}
		const SQL::Exception exception("Could not open database, the aFilename parameter was empty.");
		throw exception;
	}
