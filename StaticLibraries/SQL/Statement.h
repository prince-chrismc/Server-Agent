...
namespace SQL
{
	...
	private:
		...
		private:
			...
			sqlite3_stmt*   m_pStmt;      //!< Pointer to SQLite Statement Object
	...
	private:
		...
		inline void check(const int aRet) const
		{
			if(SQL::OK != aRet)
			{
				throw SQL::Exception(m_StmtPtr, aRet);
			}
		}
		...
		inline void checkRow() const
		{
			if(false == m_Ok)
			{
				throw SQL::Exception("No row to get a column from. executeStep() was not called, or returned false.");
			}
		}
		...
		inline void checkIndex(const int aIndex) const
		{
			...
			{
				throw SQL::Exception("Column index out of range.");
			}
		}
